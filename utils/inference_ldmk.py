...
def face_detect(images):
    ...
    if not args.nosmooth: boxes = get_smoothened_boxes(boxes, T=4)
...
def main():
    ...
    for i, (img_batch, mel_batch, frames, coords, ldmks) in enumerate(tqdm(gen, 
        ...
        for p, f, c, l in zip(pred, frames, coords, ldmks):
            ...
            # for (x,y) in l[:17]:
            #     cv2.circle(p, (x, y), 4, (255, 0, 0), -1)
            mask = np.zeros_like(p, dtype = 'float32')
            cv2.fillConvexPoly(mask, l[0:17], (1.0, 1.0, 1.0))
            ks = max(p.shape) // 5
            ...
            out.write(f)
        cv2.imwrite("temp/all.png", np.hstack([p, mask*255 , bg, f[y1:y2, x1:x2]]))
    out.release()
