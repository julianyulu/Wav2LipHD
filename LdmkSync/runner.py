...
class Runner:
    ...
    def loss_fn(self, a, v, y):
        ...
        simDist = torch.abs(y - d.unsqueeze(1)).mean()
        return loss, simDist
    ...
    def eval(self):
        ...
        while True:
            for x, mel, y in self._test_data_loader:
                ...
                y = y.to(device)
                loss, simDist = self.loss_fn(a, v, y)
                losses.append(loss.item())
                simDists.append(simDist)
                step += 1
        ...
        averaged_simDist = sum(simDists) / len(simDists)
        return {"eval_loss": averaged_loss,
                "eval_simDist": averaged_simDist,
    ...
    def train(self):
        ...
        while self.global_epoch < self.cfg.runtime.nepochs:
            ...
            for step, (x, mel, y) in prog_bar:
                ...
                running_simDist += simDist
                #pdb.set_trace()
                prog_bar.set_description(f"Epoch: {self.global_epoch} | Step: {self.global_step} | Train Loss: {running_loss / (step + 1):.6f} | simDist: {running_simDist / (step + 1):.4f}")
            ...
            self.log({"step": self.global_step,
                      "epoch": self.global_epoch,
                      "simDist": running_simDist / (step + 1),
